...
def train_model(model, train_loader, optimizer, criterion, epoch, args):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        for name, param in model.named_parameters():
            if rank == 0:
                gather_list = [torch.zeros_like(param.grad, dtype = torch.float32) for _ in range(num_nodes)]
                scatter_list =  [torch.zeros_like(param.grad, dtype = torch.float32)]
            else:
                gather_list = None
                scatter_list = None
            torch.distributed.gather(param.grad, gather_list=gather_list, dst=0, group=None, async_op=False)
            if rank == 0:
                # print(len(gather_list))
                # print(gather_list[0].shape)
                stacked_list = torch.stack(gather_list, dim = 0)
                # print(stacked_list.shape)
                mean_param = torch.mean(stacked_list, dim = 0)
                # print(mean_param.shape)
                # print("gather finished")
                scatter_list = [mean_param for _ in range(num_nodes)]
            torch.distributed.scatter(param.grad, scatter_list=scatter_list, src=0, group=None, async_op=False)
        optimizer.step()
        running_loss += loss.item()
        if batch_idx != 0:
            time_per_iteration += (datetime.now() - begin_time)
            #print(f'Time for iteration {batch_idx+1}: {(datetime.now() - begin_time).total_seconds()} seconds.')
        if batch_idx % 20 == 19:
